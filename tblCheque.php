...
//---------------------------------------------------------------------------------------------------------------------------
    ...
    $sqlRevision = "SELECT a.cod_registro, NVL(d.correlativo,0) correlativo, DECODE(a.tipo_cur,'CC','CONTABLE','CP','PRESUPUESTARIO') tipo_cur, a.cur_actual||'-'||anio_cur_actual cur_actual,
                    ...
                    DECODE(d.fecha_anula,NULL,'NO','SI') anulado, DECODE(d.fecha_autoriza,NULL,'NO','SI') autorizado
                    ...
                    WHERE a.estado = 'AU'
                    AND a.correo = 'G'
                    AND DECODE(d.fecha_autoriza,NULL,'NO','SI')='NO'
                    AND DECODE(d.fecha_anula,NULL,'NO','SI')='NO'
                    ORDER BY a.cod_registro, d.correlativo, d.fecha_cheque";
...
.navbar li{
    ...
    <br /><br /><br />
        ...
        <header class="container__header">
            <div class="wrapper__logo--interno registro--logo">
                ...
                <h1 class="titulo--interno">Consulta de Cheques</h1>
                                    ...
                                    while(($resRevision = oci_fetch_object($conRevision)) != false){                                                                                
                                        ...
                                        echo '<td style="text-align: center;">';         
                                         if ($resRevision->ANULADO=="SI"){                               
                                                echo '<a href="ediCheque.php?noRegistro='.base64_encode($cod_registro). '&usuario='.base64_encode($usuario).' "  data-toggle="tooltip" data-placement="top" title="Agregar Cheque" class="btn btn-outline-secondary btn-sm disabled"><i class="fa fa-plus"></i></a>';
                                            }else{
                                                echo '<a href="ediCheque.php?noRegistro='.base64_encode($cod_registro). '&usuario='.base64_encode($usuario).' "  data-toggle="tooltip" data-placement="top" title="Agregar Cheque" class="btn btn-outline-success btn-sm"><i class="fa fa-plus"></i></a>';                                                
                                            }
                                        echo '</td>';
                                        echo '<td style="text-align: center;">';
                                        if ($resRevision->AUTORIZADO=="SI"){
                                            echo '<input type="checkbox" checked="" name="autorizacion" id="autorizacion" title="Realizar Entrega" class="check" value="'. $cod_autoriza .'">';
                                        }
                                        else{
                                            echo '<input type="checkbox" name="autorizacion" id="autorizacion" title="Realizar Entrega" class="check" value="'. $cod_autoriza .'">';
                                        }
                                        echo '</td>';                                                                                                                
                                        echo '</tr>'; 
    ...
    <script>
    $(function(){           
        $(document).on( 'click', '#autorizacion', function(){
            ...
            $.ajax(
               {
                  ...
                  success: function (response) {
                    if(pMarca==1){
                        //alert("Autorizado...");
                            swal({type:'success', title:'Cheque Por Entregar...', text:'INFORMACION', showConfirmButton: true, confirmButtonColor: "green", confirmButtonText:'Aceptar', timer: 1000},
                                 function(){                                    
                                    location.href = 'tblCheque.php?usuario='+'<?php echo base64_encode($usuario); ?>'; 
                                 });                        
                    }else{
                        //alert("Sin Autorización");
                             swal({type:'warning', title:'Cheque No Entregado...',  text:'ADVERTENCIA', showConfirmButton: true, confirmButtonColor: "orange", confirmButtonText:'Aceptar', timer: 1000},
                             function(){
                                location.href = 'tblCheque.php?usuario='+'<?php echo base64_encode($usuario); ?>';
                             });                                             
                    }
