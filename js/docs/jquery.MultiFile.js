	...
	$.fn.MultiFile = function (options) {
		...
		this
			...
			.each(function () {
                ...
                // Estilo a la información del archivo
                //var str = "Hello World!";
                //var res = str.toUpperCase();
                
				MultiFile.wrapID = MultiFile.instanceKey;// + '_wrap'; // Wrapper ID?
				MultiFile.E.wrap('<strong><div class="MultiFile-wrap" id="' + MultiFile.wrapID + '"></div></strong>');
				...
				MultiFile.addToList = function (slave, slave_count, files) {
					...
					//$.each(files, function (i, file) {
                        ...
                        //Estilo al boton borrar por Fredy Padilla
						var
							r = $('<div style="padding-left: 150px;" class="MultiFile-label"></div>'),
							/*b = $('<br /><a style="color:red; text-decoration:none; font-size: 20px;" title="Borrar Archivo" class="MultiFile-remove fa fa-trash" href="#' + MultiFile.wrapID + '"><strong>' + MultiFile.STRING.remove + '</a></strong>')*/                            
                            b = $('<hr/><button class="MultiFile-remove btn btn-danger btn-sm float-right" href="#' + MultiFile.wrapID + ' " ' + MultiFile.STRING.remove + '><i class="fa fa-trash"></i> Remover </button>')
	...
	*/
	$.fn.MultiFile.options = { //$.extend($.fn.MultiFile, { options: {
		...
		STRING: {
			...
			toomuch: 'Los archivos seleccionados superan el tamaño maximo permitido ($size)',
